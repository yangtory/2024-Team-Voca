...
//  & 번역페이지 단어 생성
router.post('/:newvoca_seq/add_words/wordsearch?search=:search_word', async (req, res) => {
    const newvoca_seq = req.params.newvoca_seq;
    req.body.w_vseq = newvoca_seq;
...
// ------------- 수정 에서 번역하고 단어 생성하기
router.post('/:w_seq/words/update/wordsearch?search=:search_word', async (req, res) => {
    const w_seq = req.params.w_seq;
    await WORDS.update(req.body, { where: { w_seq: w_seq } });
